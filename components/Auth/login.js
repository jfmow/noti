    ...
    return (
        ...
        <>
            ...
            <div className={`w-full h-[100dvh] bg-zinc-50 text-zinc-800 relative ${classname}`}>
                <div class="absolute h-full w-full bg-[radial-gradient(#e6bded_1px,transparent_1px)] [background-size:16px_16px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_70%,transparent_100%)]"></div>
...
export function LoginInput({ ...props }) {
    return (
        <input {...props} className="bg-zinc-50 text-zinc-600  border border-zinc-100  rounded-xl p-3 w-full outline-none my-2 text-[16px] sm:text-sm min-h-[46px] readonly:opacity-50 readonly:border-zinc-400 disabled:cursor-not-allowed" />
    )
}

export function LoginButton({ ...props }) {
    return (
        <button {...props} disabled={props.loading} className="w-full bg-zinc-100  border border-zinc-200 font-semibold p-3 rounded-xl flex items-center justify-center min-h-[50px]">
            {props.loading ? (
...
export function LoginShortcutLink({ ...props }) {
    if (props.href && props.href != "") {
        return (
            <Link href={props.href} className="text-xs text-gray-600  underline text-right w-fit">{props.children}</Link>
        )
    }
    return (
        <span className="text-xs text-gray-500 text-left w-fit">{props.children}</span>
    )
