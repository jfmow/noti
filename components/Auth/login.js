...
export function LoginInput({ ...props }) {
    return (
        <input {...props} className="bg-zinc-50 text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300 border border-zinc-100 dark:border-zinc-700 rounded-xl p-3 w-full outline-none my-2 sm:text-sm min-h-[46px] readonly:opacity-50 readonly:border-zinc-400 disabled:cursor-not-allowed" />
    )
}
...
export function LoginShortcutLink({ ...props }) {
    if (props.href && props.href != "") {
        return (
            <Link href={props.href} className="text-xs text-gray-600 dark:text-gray-300 underline text-right w-full">{props.children}</Link>
        )
    }
    return (
        <span className="text-xs text-gray-500 dark:text-gray-300 text-left w-full">{props.children}</span>
    )
