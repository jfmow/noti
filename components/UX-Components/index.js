...
export function Input({ label, ...props }) {
    ...
    return (
        <div>
            {label && (
                <label className="block mb-1 font-[500]" for={id}>{label}</label>
            )}
            <input className="w-full px-1 py-2 rounded border bg-none text-zinc-800 mb-2 font-[400]" name={id} {...props} />
        </div>
    )
}
...
export function Paragraph({ ...props }) {
    return (
        <p {...props} className="text-[14px] text-zinc-500 font-[500]">
            {props?.children}
        </p>
    )
}
