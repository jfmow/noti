...
import { ToggleSwitch } from '../UX-Components';
export default function MenuBar() {
    const { pb, currentPage, setVisible, sideBarVisible, setListedPageItems, listedPageItems, setListedPageItemsFilters, listedPageItemsFilter } = useEditorContext()
    ...
    async function handleArchivePage() {
        const state = listedPageItems.find((tPage) => tPage.id === currentPage)?.archived
        const data = {
            "archived": !state
        }
        setListedPageItems(prevItems => {
            ...
            return [
                ...filteredItems.slice(0, insertIndex),
                { ...oldItem, archived: data.archived },
                ...filteredItems.slice(insertIndex)
            ];
            //return [...prevItems.filter(item => item.id !== currentPage), { ...oldItem, icon: `${e.unified}.png` }]
        })
        ...
        <>
            <div className={styles.container}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <ToolTipCon>
                        <ToolTipTrigger>
                            <button onClick={() => setFiltersModal(!filtersModal)} type='button' className={styles.optionButton}>
                    ...
                    <ToolTipCon>
                        ...
                        <PopUpCardDropMenuStaticPos animationOrgin={'top right'} mobilepos={{
                            ...
                            >
                                ...
                                <PopUpCardDropMenuSectionItem onClick={() => handleArchivePage()}>
                                    {listedPageItems.find((tPage) => tPage.id === currentPage)?.archived ? (
                                        <>
            ...
            {filtersModal && (
                <Modal close={() => setFiltersModal(false)}>
                    <h1 style={{ marginBottom: '1em' }}>Filter pages</h1>
                    <ToggleSwitch enabled={listedPageItemsFilter.archived} onChange={() => {
                        setListedPageItemsFilters(prevItems => {
                            return {
                                ...prevItems,
                                archived: !listedPageItemsFilter.archived
                            }
                        }
                        )
                    }}>Archived pages</ToggleSwitch>
                    <ToggleSwitch enabled={listedPageItemsFilter?.archivedInTree} onChange={() => {
                        setListedPageItemsFilters(prevItems => {
                            return {
                                ...prevItems,
                                archivedInTree: !listedPageItemsFilter?.archivedInTree
                            }
                        }
                        )
                    }}>Show archived pages in tree</ToggleSwitch>
                </Modal>
            )}
        </>
    );
