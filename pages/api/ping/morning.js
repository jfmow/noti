...
export default async function sendNotif(req, res) {
    ...
    if (req.headers.authorisation === process.env.MORNING_PING_KEY) {
        try {
            ...
            const state = await sendNotifications(data.items, { "title": 'Good morning! ☀️', "body": `${new Date().toLocaleDateString()}. Time to get writing!` });
