...
export function ModalContainer({ events, noanimate, noblur, children }) {
    ...
    return (
        <motion.div initial={{ opacity: 0 }}
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }} className={`${styles.modal_container} ${noblur && styles.noblur}`} onClick={!noblur ? (events) : (null)}>
    ...
    return (
        <div className={styles.cbxcon}>
            ...
            <label className={styles.cbx}>
                <input className={styles.input} onChange={chngevent} checked={checked} id="ch1" type="checkbox" />
                <div className={styles.cbxtrans}></div>
            </label>
        </div>
    )
}
    ...
    return (
        <>
            <div className={`${className} ${styles.copy_paste_textarea}`}>
                <span>{children}</span>
                <button type='button' onClick={click}><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px"><path d="M0 0h24v24H0V0z" fill="none" /><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z" /></svg></button>
            </div>
        </>
    )
}
