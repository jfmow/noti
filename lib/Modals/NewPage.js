...
export default function NewPageModal({ pageId }) {
    return (
        <>
            {createPortal(
                <>
                    <div onClick={() => Router.push(window.location.pathname)} className="backdrop-brightness-50  fixed top-0 left-0 z-[1] flex items-center justify-center w-full h-screen ">
                        <div onClick={(e) => e.stopPropagation()} className="animate-fade-in-slow-middle z-[2] relative w-[900px] max-w-[80%] bg-[var(--background)] h-[90vh] shadow-xl rounded-xl overflow-hidden">
