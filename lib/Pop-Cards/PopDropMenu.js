...
export function PopUpCardDropMenuStaticPos({ event, ref, style, mobilepos, animationOrgin, children }) {
    ...
    return (
        <>
            <div
                style={{ ...style, transformOrigin: isMobile ? event?.clientX > (window.innerWidth / 2) ? animationOrgin.includes('bottom') ? 'bottom right' : 'top right' : animationOrgin.includes('bottom') ? 'bottom left' : 'top left' : animationOrgin, WebkitTransformOrigin: animationOrgin, right: event?.clientX > (window.innerWidth / 2) ? 0 : '', left: event?.clientX < (window.innerWidth / 2) ? 0 : '' }
                }
                className={` ${!visible && styles.hidden} ${styles.PopUpCardDropMenu} `}
                ref={node}
            >
