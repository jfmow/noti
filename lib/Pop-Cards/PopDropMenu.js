...
//Goes where mouse is
export function PopUpCardDropMenu({ event, style, minGap, children }) {
...
export function PopUpCardDropMenuStaticPos({ event, ref, style, mobilepos, children }) {
    const [visible, setVisible] = useState(false);
    const [isMobile, setIsMobile] = useState(false);
    const node = useRef();
...
export function PopUpCardDropMenuSectionItem({ onClick, onHover, onHoverLeave, children }) {
    return (
        <div onClick={onClick} onMouseEnter={onHover} onMouseLeave={onHoverLeave} className={styles.PopUpCardDropMenuSectionItem}>
            {children}
        </div>
    )
}