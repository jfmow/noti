import styles from '@/styles/pop/PopUpCard.module.css';
import { useEffect, useRef, useState } from 'react';

export default function PopUpCard({ className, style, event, children }) {
...
export function PopUpCardCorner({ event, children }) {
    const [visible, setVisible] = useState(false)
    const cardWidth = 520;
    const cardHeight = 400;
    ...
    return (
        <>
            <div
                ...
                className={`${!visible && styles.hidden} ${styles.PopUpCard}`}
                ref={node}
            >
                <div className={styles.closebtn} onClick={() => setVisible(false)}><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px"><path d="M0 0h24v24H0V0z" fill="none" /><path d="M18.3 5.71c-.39-.39-1.02-.39-1.41 0L12 10.59 7.11 5.7c-.39-.39-1.02-.39-1.41 0-.39.39-.39 1.02 0 1.41L10.59 12 5.7 16.89c-.39.39-.39 1.02 0 1.41.39.39 1.02.39 1.41 0L12 13.41l4.89 4.89c.39.39 1.02.39 1.41 0 .39-.39.39-1.02 0-1.41L13.41 12l4.89-4.89c.38-.38.38-1.02 0-1.4z" /></svg></div>
                {children}
...
//Items
export function PopUpCardTitle({ children }) {
    return (
        <div className={styles.PopUpCardTitle}>
            <h2 className={styles.PopUpCardTitle_text}>{children}</h2>
        </div>
    )
}

export function PopUpCardSubTitle({ children }) {
    return (
        <div className={styles.PopUpCardSubTitle}>
            <h5 className={styles.PopUpCardSubTitle_text}>{children}</h5>
        </div>
    )
}

export function PopUpCardScrollCon({ children }) {
    return (
        <div className={styles.PopUpCardScrollCon}>
            {children}
        </div>
    )
}
