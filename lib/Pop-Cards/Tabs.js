...
export function TabsProvider({ defaultOpen = false, children }) {
    ...
    useEffect(() => {
        if (tabgroupRef.current && triggerRef.current) {
            ...
            if (window.matchMedia("(max-width: 600px)").matches) {
                tabgroupRef.current.style.top = "unset"
                ...
                <>
                    {createPortal(
                        <>
                            ...
                            <div className="fixed top-0 left-0 bg-[#ffffff] shadow z-[14] border rounded-lg flex flex-col items-center w-[400px] min-h[200px] text-[#000000] overflow-hidden max-h-[50dvh] " ref={tabgroupRef} {...props}>
                                {props.children}
                            </div>
                        </>,
                        document.body
...
export function TabMenuItem({ ...props }) {
    ...
    return (
        ...
        }} className={`${visibleItem === props.for ? "bg-[#eaeaea]" : ''} ${"w-full flex items-center justify-center p-2 cursor-pointer rounded-lg font-[600] text-[14px] min-h-[37px]"}`} {...props}>
            {props.children}
        </div>
    )
}
