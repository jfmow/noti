...
export function DropDownTrigger({ hover, fullw, children }) {
    ...
    return (
        <div ref={triggerRef} onClick={() => setDropDownVisible(true)} className={`flex items-center justify-center  h-fit ${fullw ? "w-full" : "w-fit"}`}>
            {children}
        </div>
    )
}
                ...
                <>
                    {createPortal(
                        <>
                            ...
                            <div ref={dropDownRef} className="fixed flex flex-col w-[200px] bg-[var(--popupBackground)] z-[11] shadow-sm rounded-lg">
                                {children}
                            </div>
                        </>,
                        document.body
...
export function DropDownSection({ children }) {
    return (
        <div className={"border-t-[#00000047] border-t w-full h-fit p-[4px] text-[14px] font-[500]"}>
            {children}
        </div>
    )
}
...
export function DropDownItem({ ...props }) {
    return (
        <div className="flex items-center px-[8px] py-[6px] text-[var(--modalTextColor)] [&>svg]:w-4 [&>svg]:h-4 [&>svg]:mr-2 [&>svg]:flex cursor-pointer hover:bg-[var(--popupButtonBg)] rounded" {...props}>
            {props.children}
        </div>
    )
}
    ...
    useEffect(() => {
        if (dropDownRef2.current && triggerRef.current) {
            ...
            if (((mainDropDown.left + mainDropDown.right) / 2) < viewPortWidth / 2) {
                //LEFT
                x = mainDropDown.right - 7
                transformOrigin = "left"
            } else {
                //RIGHT
                x = mainDropDown.left - dropdown.width + 7
    ...
    return (
        <>
            {visible && (
                <>
                    {createPortal(
                        <div ref={dropDownRef2} className="fixed flex flex-col w-[200px] bg-[var(--popupBackground)] z-[18] shadow-sm rounded-lg">
                            {children}
                        </div>,
                        document.body
                    )}
