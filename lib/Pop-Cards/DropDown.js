...
export function DropDownContainer({ children }) {
    ...
    return (
        <PopUpCardDropMenuContext.Provider value={{ dropDownVisible, setDropDownVisible, containerRef, dropDownRef, triggerRef }}>
            {dropDownVisible && (
                <div onClick={() => setDropDownVisible(false)} className={"fixed top-0 left-0 right-0 bottom-0 w-full h-screen z-[11]"} />
            )}
...
export function DropDown({ children }) {
    ...
    return (
        <>
            {dropDownVisible && (
                <div ref={dropDownRef} className="fixed flex flex-col w-[200px] bg-[var(--popupBackground)] z-[18] shadow-sm rounded-lg border">
                    {children}
                </div>
            )}
        </>
