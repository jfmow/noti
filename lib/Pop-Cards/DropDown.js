...
export function DropDownSectionTitle({ ...props }) {
    return (
        <h1 className="font-[600] text-[14px] p-4 text-[#000000]" {...props}>{props.children}</h1>
    )
}

export function DropDownItem({ className, ...props }) {
    return (
        <div className={`flex active:border-zinc-300 border border-transparent items-center px-[8px] py-[6px] text-[#000000] [&>svg]:w-4 [&>svg]:h-4 [&>svg]:mr-2 [&>svg]:flex cursor-pointer hover:bg-[#eaeaea] rounded ${className}`} {...props}>
            {props.children}
        </div>
    )
}
