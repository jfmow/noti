            ...
            {visible && (
                <>
                    {ReactDom.createPortal(
                        <>
                            <div onClick={() => closePopUp()} className="fixed top-0 left-0 right-0 bottom-0 h-screen w-full z-[19] select-none" />
                            <div ref={popUpRef} className="fixed bg-[var(--popupBackground)] shadow-sm z-[20] border rounded-lg p-[1.5rem] flex flex-col items-center w-fit min-w-[200px] min-h-[200px] h-fit text-[var(--modalTextColor)] [&>h1]:text-[18px]" {...props}>
                                {props.children}
                            </div>
                        </>,
                        document.body
...
export function PopUpCardsGlobalButton({ click, style, classname, disabled, children }) {
    return (
        <button style={style} disabled={disabled} className="flex items-center justify-center min-w-[60px] h-[30px] rounded border-none cursor-pointer bg-slate-100 text-slate-800 px-[8px] py-[16px]" type="button" onClick={click}>
            {children}
        </button >
    )
}