            ...
            {visible && (
                <>
                    {ReactDom.createPortal(
                        <>
                            <div onClick={() => closePopUp()} className="fixed top-0 left-0 right-0 bottom-0 h-screen w-full z-[15] select-none" />
                            <div ref={popUpRef} className="fixed bg-[var(--popupBackground)] shadow-sm z-[16] border rounded-lg p-[1.5rem] flex flex-col items-center w-fit min-w-[200px] min-h-[200px] h-fit text-[var(--modalTextColor)] [&>h1]:text-[18px]" {...props}>
                                {props.children}
                            </div>
                        </>,
                        document.body
